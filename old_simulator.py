...
def test_anySimulator(s):
	...
	#s.getOutputValue(pos,vel) Simulator shouldn't have functionality to do this, only SoundGenerator. Simulators can simulate arbitrary physical systems, including ones without any reasonable way to define a single real number output value.
...
def getSimListInAnyState(simulator, timeStep, stepNum, inbetweenSteps, stepfunc, initPos, initVel, initTime):
	...
	simulator is the object which defines the initial position, velocity (and time), but we can set another state with the initPos,initVel, and initTime values
	...
	outputList = [getDefaultOutput(currentPos, currentVel, currentTime)]
	for i in range(stepNum):
		(currentPos,currentVel) = stepfunc(simulator, timeStep, currentPos, currentVel, currentTime)
		currentTime += timeStep
		outputList.append(getDefaultOutput(currentPos, currentVel, currentTime))
...
def getSimListDefaultState(simulator, timeStep, stepNum, inbetweenSteps, stepfunc):
	return getSimListInAnyState(simulator, timeStep, stepNum, inbetweenSteps, stepfunc, simulator.getDefaultInitPos(), simulator.getDefaultInitVel(), simulator.getDefaultInitTime())
